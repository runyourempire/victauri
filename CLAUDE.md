...
X-ray vision and hands for AI agents inside Tauri apps. A browser tool with JS eval can *poke* a Tauri backend (Tauri exposes `window.__TAURI_INTERNALS__.invoke`, so it can invoke commands) — but only Victauri can *read* it safely. The database, the command registry, and process/memory state have no `eval_js` equivalent — those backend tools run read-only through direct `AppHandle` access, independent of the webview. (The IPC history with response bodies is a *different* mechanism: it's derived from the JS bridge's `fetch` interception, so it IS eval-based and needs a live webview — its edge is capturing bodies + the real `Err` status the browser Performance API hides, not direct backend access. Don't claim it as a non-webview channel.) On macOS WKWebView / Linux WebKitGTK there's no CDP surface, so CDP-based tooling like Playwright can't attach — but those webviews DO have their own remote inspection (WebKit Inspector Protocol, `WebKitWebDriver`, `webinspectord`), so the honest moat is "one integration → both sides of the IPC boundary, identical on all 3 platforms," NOT "nothing else can get in." Victauri gives agents simultaneous, read-only access to the webview DOM, the Rust backend, the IPC layer, the database, and native window state — all through a single MCP interface.
