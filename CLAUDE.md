...
- **Discovery session tokens** — `start_server()` always generates a UUID session token and writes it to `<temp>/victauri/<pid>/token` (user-only permissions on Unix). When auth is explicitly configured, uses that token instead. `VictauriClient::discover()` reads the token file and includes it as Bearer header. Auth middleware remains opt-in — the token is harmlessly ignored when auth is off, but ready for instant activation.
